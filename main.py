...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        logger.warning(
            f"⚠️ Vector DB not pre-loaded: {str(e)}. "
            "(Upload a file via /api/v1/ingest to initialize)."
        )
...
app = FastAPI(
    ...
    description=(
        "Production-grade Telecom Customer Support RAG Microservice "
        "built with FastAPI, LangChain, FAISS, and Gemini."
    ),
    lifespan=lifespan
)
